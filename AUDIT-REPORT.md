...
**Refactor after review**: inventory consolidated to a single `/admin/inventory` page with two tabs — `Stock overview` and `Movements` — styled by a shared full-width segmented `AdminTabs` control (also used by Orders' Active/Archived tabs). Stock level (Healthy / Low / Out of stock) is folded into the filter sheet (`?status=` deep-link supported); the separate `low-stock` page and standalone movements route were removed. Sidebar reorganized into Operations / Catalog / Inventory / Store management / Finance / Communication / System.
