    ...
    <ResponsiveDataTable label="Orders table horizontal scroll">
      <table className="w-full min-w-[1480px] whitespace-nowrap text-left text-sm">
        <thead className="sticky top-0 z-10 border-b border-line bg-sage/35 text-xs uppercase tracking-[0.12em] text-muted">
          <tr>
            {columns.map((column) => (
              <th className="px-5 py-4 font-bold" key={column}>{column}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-line">
          {orders.map((order) => (
            <tr className="group hover:bg-cream/60" key={order.orderNumber}>
              ...
              <td className="px-5 py-4"><StatusBadge status={order.fulfillmentMethod === 'PICKUP' ? 'Pickup' : 'Delivery'} className={fulfillmentClass(order.fulfillmentMethod)} /></td>
              <td className="px-5 py-4"><StatusBadge status={order.shoppingMode === 'WHOLESALE' ? 'Wholesale' : 'Retail'} className={shoppingModeClass(order.shoppingMode)} /></td>
              <td className="px-5 py-4"><StatusBadge status={order.paymentStatus} className={statusClass(order.paymentStatus)} /></td>
              <td className="px-5 py-4"><StatusBadge status={formatOrderStatus(order.orderStatus)} className={statusClass(order.orderStatus)} /></td>
