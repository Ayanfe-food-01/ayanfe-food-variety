...
function ReferenceValue({ reference }: { reference: string | null }) {
  return reference
    ? <span className="block min-w-0 truncate max-w-[280px] break-all font-semibold text-green-dark lg:break-normal">{reference}</span>
            ...
            {payments.map((payment) => (
              <tr className="group hover:bg-cream/60" key={payment.id}>
                <td className="px-5 py-4">
                  <Link className="block min-w-0 truncate max-w-[190px] font-semibold text-green hover:text-orange" to={`/admin/orders/${payment.orderNumber}`}>{payment.orderNumber}</Link>
                  <p className="block min-w-0 truncate mt-1 max-w-[280px] text-xs text-muted">{payment.customerName} · {payment.customerEmail ?? payment.customerPhone}</p>
                </td>
