...
export function CheckoutSummary({
  ...
  return (
    <aside className="min-w-0 rounded-2xl border border-line bg-white p-6 shadow-sm sm:p-8 lg:sticky lg:top-40" aria-labelledby="checkout-summary-heading">
      ...
      <div className="my-6 space-y-3 border-y border-line py-5 text-sm">
        ...
        <div className="flex justify-between gap-4 text-muted"><span>{fulfillmentMethod === 'PICKUP' ? 'Pickup fee' : 'Delivery fee'}</span><span className="font-bold text-green-dark">{deliveryFee === null ? '—' : deliveryFee === 0 ? 'FREE' : formatPrice(deliveryFee)}</span></div>
      </div>
      <div className="mb-5 rounded-xl bg-sage/35 p-3 text-xs leading-5 text-muted">
        <strong className="text-green-dark">{fulfillmentMethod === 'PICKUP' ? 'Pickup selected.' : fulfillmentMethod === 'DELIVERY' ? 'Delivery selected.' : 'Choose pickup or delivery.'}</strong>{' '}
        {fulfillmentMethod === 'PICKUP'
          ...
          : fulfillmentMethod === 'DELIVERY'
            ? (isZoneResolving ? 'Checking your delivery zone…' : resolvedZone ? `Your delivery fee is based on the ${resolvedZone.label} zone.` : 'Delivery is unavailable for your selected city.')
