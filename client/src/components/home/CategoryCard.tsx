...
export function CategoryCard({ category }: CategoryCardProps) {
  return (
    <Link className="relative min-h-[180px] overflow-hidden rounded-2xl bg-green transition-transform duration-300 hover:-translate-y-1 sm:min-h-[220px]" to="/shop">
      {category.imageUrl && <img className="size-full object-cover transition-transform duration-500 hover:scale-105" src={category.imageUrl} alt={`${category.name} products`} />}
