    ...
    <div className={`product-search-autocomplete ${className}`} ref={wrapperRef}>
      <form className="search-form" onSubmit={handleSubmit} role="search">
        ...
        <input
          ...
          onFocus={() => { if (canShowSuggestions) setIsOpen(true) }}
        ...
        <div className="product-search-dropdown" id={listboxId} role="listbox" aria-label="Product suggestions">
          {isLoading ? (
            ...
            suggestions.map((product, index) => (
              ...
              >
                <span className="product-search-option-name">{product.name}</span>
                <span className="product-search-option-meta">{product.category}{product.unit ? ` · ${product.unit}` : ''}</span>
              </button>
            ))
          ) : (
