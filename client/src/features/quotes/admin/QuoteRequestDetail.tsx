...
import {
  deliveryFeeModeFromQuote,
  isValidDeliveryFee,
  isValidUnitPrice,
  MAX_DELIVERY_FEE,
  MAX_UNIT_PRICE,
  OTHER_REASON_KEY,
  type QuoteDeliveryFeeOption,
  type QuoteFulfillmentOption,
} from './detail/constants'
import { QuoteDetailCustomerCard } from './detail/QuoteDetailCustomerCard'
import { QuoteDetailCustomerResponse } from './detail/QuoteDetailCustomerResponse'
import { QuoteDetailHeader } from './detail/QuoteDetailHeader'
import { QuoteDetailItemsTable } from './detail/QuoteDetailItemsTable'
import { QuoteDetailModals } from './detail/QuoteDetailModals'
import { QuoteDetailPrepareSection } from './detail/QuoteDetailPrepareSection'
import { QuoteDetailSnapshotSection } from './detail/QuoteDetailSnapshotSection'
import { QuoteDetailStatusSection } from './detail/QuoteDetailStatusSection'
import { QuoteStatusStepper } from './detail/QuoteStatusStepper'
...
export function QuoteRequestDetail() {
  ...
  const [deliveryFeeMode, setDeliveryFeeMode] = useState<QuoteDeliveryFeeOption>('ZONE')
  ...
  const syncFromLoaded = (loaded: AdminQuoteRequestDetail) => {
    ...
    const feeMode = deliveryFeeModeFromQuote(loaded.deliveryFeeMode, loaded.deliveryFee)
    setDeliveryFeeMode(loaded.fulfillmentMethod === 'DELIVERY' ? feeMode : 'ZONE')
  }
  ...
  const handleDeliveryFeeModeChange = (value: QuoteDeliveryFeeOption) => {
    setDeliveryFeeMode(value)
    if (value !== 'CUSTOM') setDeliveryFeeInput('')
  }
  ...
  const persistQuotation = async () => {
    ...
    // ZONE and FREE lock the fee to the zone rate / zero respectively.
    // CUSTOM requires an explicit amount; a blank amount is rejected here so
    // the store never locks an unintended fee.
    if (fulfillmentMethod === 'DELIVERY' && deliveryFeeMode === 'CUSTOM') {
      if (!isValidDeliveryFee(deliveryFeeInput)) {
        setQuotationError('Enter the custom delivery fee (a non-negative amount with up to two decimal places).')
        return
      }
      if (Number(deliveryFeeInput) > MAX_DELIVERY_FEE) {
    ...
    try {
      const updated = await prepareAdminQuotePricing(reference, {
        ...
        deliveryFee: fulfillmentMethod === 'PICKUP' || deliveryFeeMode !== 'CUSTOM' ? '' : deliveryFeeInput.trim(),
        fulfillmentMethod,
        deliveryFeeMode: fulfillmentMethod === 'PICKUP' ? undefined : deliveryFeeMode,
      })
      setQuote(updated)
      setStatus(updated.status)
      syncFromLoaded(updated)
  ...
  const previewDeliveryFeeCents = deliveryFeeMode === 'CUSTOM' && Number.isFinite(Number(deliveryFeeInput)) && Number(deliveryFeeInput) > 0
    ...
    <div>
      ...
      {canPrepareQuotation && (
        <QuoteDetailPrepareSection
          ...
          onDeliveryFeeModeChange={handleDeliveryFeeModeChange}
          ...
          deliveryFeeIsBlank={deliveryFeeMode === 'CUSTOM' && deliveryFeeInput.trim() === ''}
