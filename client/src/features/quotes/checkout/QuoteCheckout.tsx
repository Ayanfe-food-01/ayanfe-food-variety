...
export function QuoteCheckout() {
  ...
  useEffect(() => {
    ...
    getCustomerQuoteRequest(reference)
      .then((result) => {
        ...
        } else if (result.fulfillmentMethod === 'PICKUP') {
          setForm((current) => ({ ...current, fulfillmentMethod: 'PICKUP' }))
        }
      })
  ...
  const subtotal = quote?.quotedSubtotal !== null && quote?.quotedSubtotal !== undefined
    ...
    : 0
  const feeMode = quote?.deliveryFeeMode ?? null
  const lockedQuoteFee = quote?.deliveryFee !== null && quote?.deliveryFee !== undefined ? Number(quote.deliveryFee) : null
  // A prepared quotation locks its delivery fee by mode: FREE fixes zero,
  // CUSTOM and ZONE reuse the fee stored on the quotation. Legacy quotes
  // (mode null) keep the old behaviour: a stored fee is an override, otherwise
  // the customer's delivery zone prices it exactly like a normal checkout.
  const zoneFee = deliveryFeeFromZone(resolvedZone, subtotal)
  let effectiveDeliveryFee: number | null = 0
  let hasOverride = false
  if (isDelivery) {
    if (feeMode === 'FREE') {
      effectiveDeliveryFee = 0
      hasOverride = true
    } else if (feeMode === 'CUSTOM' || feeMode === 'ZONE') {
      effectiveDeliveryFee = lockedQuoteFee ?? 0
      hasOverride = true
    } else if (lockedQuoteFee !== null) {
      effectiveDeliveryFee = lockedQuoteFee
      hasOverride = true
    } else {
      effectiveDeliveryFee = zoneFee
    }
  }
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (isDelivery) {
      ...
      // The zone is resolved whenever the fee is not a locked override, and
      // also for ZONE-mode quotes so the chosen location can be compared with
      // the zone the quotation was priced against.
      const needsZone = feeMode === 'ZONE' || !hasOverride
      if (form.city.trim() && needsZone) {
        if (isZoneResolving) {
        ...
        } else if (feeMode === 'ZONE' && quote?.deliveryZoneId && resolvedZone.id !== quote.deliveryZoneId) {
          nextErrors.city = 'The delivery location for this quotation has changed. Contact the store for a revised quotation.'
        }
      }
    }
