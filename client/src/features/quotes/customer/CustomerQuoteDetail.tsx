...
import { AcceptQuoteDialog } from './detail/AcceptQuoteDialog'
import { DeclineQuoteDialog } from './detail/DeclineQuoteDialog'
import { OTHER_REASON_KEY } from './detail/declineReasons'
import { QuoteActionPanel } from './detail/QuoteActionPanel'
import { QuoteStatusBadge } from './detail/QuoteStatusBadge'
...
export function CustomerQuoteDetail() {
  ...
  const [declineReasonOption, setDeclineReasonOption] = useState<string | null>(null)
  ...
  const confirmDecline = async () => {
    if (!reference || !quote) return
    if (!declineReasonOption) {
      setActionError('Please select a reason for declining.')
      return
    }
    const specifiedReason = declineReason.trim()
    if (declineReasonOption === OTHER_REASON_KEY && !specifiedReason) {
      setActionError('Please provide a reason for declining.')
      return
    }
    const finalReason = declineReasonOption === OTHER_REASON_KEY ? specifiedReason : declineReasonOption
    setIsBusy(true)
    setActionError(null)
    try {
      setQuote(await rejectCustomerQuoteRequest(reference, finalReason))
      closeDialogs()
    } catch (caught: unknown) {
  ...
  const deliveryDayRange = formatQuoteDayRange(quote?.deliveryMinDays ?? null, quote?.deliveryMaxDays ?? null)
  const deliveryLocation = [quote?.state, quote?.city].filter(Boolean).join(', ')
  ...
  return (
    <>
              ...
              {hasPricing && quote.quotedSubtotal !== null && quote.quotedTotal !== null && (
                <div className="mt-5 space-y-3 border-t border-line pt-5 text-sm">
                  ...
                  <div className="flex justify-between text-muted">
                    <span>{quoteFeeLabel(quote.fulfillmentMethod, quote.deliveryFeeMode, quote.deliveryAreaName)}</span>
                    <strong className="text-green-dark">{formatQuoteDeliveryFee(quote.fulfillmentMethod, quote.deliveryFee)}</strong>
                  </div>
                  {isDelivery && (
                    <div className="space-y-1.5 rounded-xl bg-cream/50 p-4 text-xs leading-5 text-muted">
                      <p>
                        {deliveryLocation
                          ? `Delivery to ${deliveryLocation}${quote.deliveryAddress ? ` — ${quote.deliveryAddress}` : ''}.`
                          : 'Deliver to your address at checkout.'}
                      </p>
                      {deliveryDayRange && (
                        <p>
                          Estimated delivery: <strong className="text-green-dark">{deliveryDayRange}</strong>.
                        </p>
                      )}
                      <p>
                        {quote.deliveryFee === null
                          ? 'The delivery fee is calculated from your delivery zone and added to your total at checkout.'
                          : 'The delivery fee is included in your quoted total.'}
                      </p>
                    </div>
                  )}
                  {quote.fulfillmentMethod === 'PICKUP' && <p className="text-xs text-muted">Collect from the store when it is ready. No delivery fee applies.</p>}
      ...
      {isDeclineDialogOpen && quote && (
        <DeclineQuoteDialog
          reasonOption={declineReasonOption}
          onSelectReason={(value) => { setDeclineReasonOption(value); setActionError(null) }}
          reason={declineReason}
          onChangeReason={(value) => { setDeclineReason(value); setActionError(null) }}
          error={actionError}
          isBusy={isBusy}
