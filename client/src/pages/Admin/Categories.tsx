              ...
              <tbody className="divide-y divide-line">
                {categories.map((category) => (
                   <tr key={category.id} className="group">
                     <td className="w-[400px] max-w-[400px] overflow-hidden px-5 py-4"><div className="flex min-w-[360px] max-w-[368px] items-center gap-3"><div className="size-14 shrink-0 overflow-hidden rounded-xl bg-sage">{category.imageUrl && <img className="size-full object-cover" src={category.imageUrl} alt="" />}</div><div className="min-w-0"><p className="block min-w-0 truncate font-bold text-green-dark">{category.name}</p><p className="block min-w-0 truncate mt-1 text-xs text-muted">{category.description || 'No description'}</p><p className="block min-w-0 truncate mt-1 text-xs text-muted">{category.slug}</p></div></div></td>
