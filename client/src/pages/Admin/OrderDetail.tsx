      ...
      <div className="mt-8 grid gap-5 xl:grid-cols-[1.4fr_0.8fr]">
        <div className="space-y-5">
          ...
          <section className="rounded-2xl border border-line bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-3 space-y-2 border-t border-line pt-4 text-sm"><div className="flex justify-between text-muted"><span>Subtotal</span><span>{formatPrice(order.subtotal)}</span></div>{order.fulfillmentMethod === 'DELIVERY' && order.deliveryZoneName && <div className="flex justify-between text-muted"><span>Delivery zone</span><span className="font-bold text-green-dark">{order.deliveryZoneName}</span></div>}<div className="flex justify-between text-muted"><span>Delivery fee</span><span className="font-bold text-green-dark">{order.deliveryFee === '0' || Number(order.deliveryFee) === 0 ? 'FREE' : formatPrice(order.deliveryFee)}</span></div><div className="flex justify-between pt-2 text-base font-bold text-green-dark"><span>Total</span><span>{formatPrice(order.total)}</span></div></div>
          </section>
