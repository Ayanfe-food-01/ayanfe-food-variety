        ...
        <div className="space-y-5">
          <section className="rounded-2xl border border-line bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div><p className="text-xs text-muted">Payment method</p><p className="mt-1 font-bold text-green-dark">{order.paymentMethod === 'PAYSTACK' ? 'Paystack' : 'Bank transfer'}</p></div>
            </div>
            {order.fulfillmentMethod === 'DELIVERY' ? (
              <div className="mt-5 rounded-xl bg-sage/35 p-4 text-sm">
                ...
                <p className="mt-1 font-bold text-green-dark">{order.deliveryAddress}, {order.deliveryAreaName ? `${order.deliveryAreaName}, ` : ''}{order.city}{order.state ? `, ${order.state}` : ''}</p>
              </div>
            ) : (
          ...
          <section className="rounded-2xl border border-line bg-white p-5 shadow-sm sm:p-6">
            <h2 className="text-lg font-bold text-green-dark">Order items</h2>
              <div className="mt-4 divide-y divide-line">{order.orderItems.map((item) => <div className="flex items-start justify-between gap-4 py-4 text-sm" key={item.id}><div><p className="font-bold text-green-dark">{item.productName}</p>{item.productOptionLabel && <p className="mt-0.5 text-xs font-semibold text-orange">{item.productOptionLabel}</p>}{item.wholesalePackageName && <p className="mt-0.5 text-xs font-semibold text-orange">{item.wholesalePackageName}{item.wholesaleUnitsPerPackage ? ` · ${item.wholesaleUnitsPerPackage} ${item.wholesaleUnitsPerPackage === 1 ? 'unit' : 'units'} per package` : ''}</p>}<p className="mt-1 text-muted">{item.quantity} × {formatPrice(item.unitPrice)}</p></div><p className="font-bold text-green-dark">{formatPrice(item.subtotal)}</p></div>)}</div>
            <div className="mt-3 space-y-2 border-t border-line pt-4 text-sm"><div className="flex justify-between text-muted"><span>Subtotal</span><span>{formatPrice(order.subtotal)}</span></div>{order.fulfillmentMethod === 'DELIVERY' && order.deliveryZoneName && <div className="flex justify-between text-muted"><span>Delivery zone</span><span className="font-bold text-green-dark">{order.deliveryZoneName}</span></div>}{order.fulfillmentMethod === 'DELIVERY' && order.deliveryAreaName && <div className="flex justify-between text-muted"><span>Delivery area</span><span className="font-bold text-green-dark">{order.deliveryAreaName}</span></div>}{order.fulfillmentMethod === 'DELIVERY' && order.state && <div className="flex justify-between text-muted"><span>State</span><span className="font-bold text-green-dark">{order.state}</span></div>}{order.fulfillmentMethod === 'DELIVERY' && order.deliveryMinDays && order.deliveryMaxDays && <div className="flex justify-between text-muted"><span>Estimated delivery</span><span className="font-bold text-green-dark">{order.deliveryMinDays === order.deliveryMaxDays ? `${order.deliveryMinDays} business day${order.deliveryMinDays === 1 ? '' : 's'}` : `${order.deliveryMinDays}–${order.deliveryMaxDays} business days`}</span></div>}<div className="flex justify-between text-muted"><span>Delivery fee</span><span className="font-bold text-green-dark">{order.deliveryFee === '0' || Number(order.deliveryFee) === 0 ? 'FREE' : formatPrice(order.deliveryFee)}</span></div><div className="flex justify-between pt-2 text-base font-bold text-green-dark"><span>Total</span><span>{formatPrice(order.total)}</span></div></div>
          </section>
