...
import { assertEmailConfigured, EmailServiceError, escapeHtml, renderBrandedEmail, sendEmail, } from '../../lib/email/email.service.js';
...
const getPasswordResetUrl = (token) => {
    if (!env.publicAppUrl?.trim()) {
        throw new PasswordResetEmailError('configuration', 'PUBLIC_APP_URL is not configured on the server.');
    }
    let appUrl;
    try {
        ...
        throw new PasswordResetEmailError('configuration', 'PUBLIC_APP_URL is not a valid URL.');
    }
    if (!['http:', 'https:'].includes(appUrl.protocol)) {
        throw new PasswordResetEmailError('configuration', 'PUBLIC_APP_URL must use HTTP or HTTPS.');
    }
...
export async function sendPasswordResetEmail(input) {
    try {
        ...
        throw new PasswordResetEmailError('network', 'The email provider could not be reached.');
    }
}
