...
import { createBanner, deleteBanner, getAdminBanner, getPublicBanners, listAdminBanners, updateBanner, updateBannerStatus, } from './banner.service.js';
...
export const bannerImageUpload = (request, response, next) => {
...
export const getPublicBannersController = async (_request, response) => {
    response.json({ success: true, data: { banners: await getPublicBanners() } });
};
export const listAdminBannersController = async (_request, response) => {
    response.json({ success: true, data: await listAdminBanners() });
};
export const getAdminBannerController = async (request, response) => {
    response.json({ success: true, data: { banner: await getAdminBanner(validateBannerId(request.params.id)) } });
};
export const createAdminBannerController = async (request, response) => {
    if (!request.file)
        throw new HttpError(400, 'A banner image is required.');
    let image;
    try {
        ...
        if (image)
            await deleteBannerImage(image.publicId);
        throw error;
    }
};
export const updateAdminBannerController = async (request, response) => {
    ...
    try {
        ...
        if (image && existingBanner.imagePublicId !== image.publicId)
            await deleteBannerImage(existingBanner.imagePublicId);
...
export const updateAdminBannerStatusController = async (request, response) => {
    response.json({
        success: true,
...
export const deleteAdminBannerController = async (request, response) => {
    const publicId = await deleteBanner(validateBannerId(request.params.id));
    const removed = await deleteBannerImage(publicId);
    if (!removed)
        console.warn(JSON.stringify({ event: 'banner_cloudinary_delete_failed', publicId }));
