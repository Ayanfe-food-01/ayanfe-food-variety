...
export function validateBannerInput(body) {
    if (!isRecord(body))
        throw new HttpError(400, 'Banner data is required.');
    return {
...
export function validateBannerId(value) {
    const id = Array.isArray(value) ? value[0] : value;
    if (!id || !UUID_PATTERN.test(id.trim()))
        throw new HttpError(400, 'Banner ID is invalid.');
    return id.trim();
}
export function validateBannerStatusInput(body) {
    if (!isRecord(body))
        throw new HttpError(400, 'Banner status is required.');
