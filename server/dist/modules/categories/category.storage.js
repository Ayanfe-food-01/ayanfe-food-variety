...
export async function uploadCategoryImage(file) {
    if (file.size > MAX_CATEGORY_IMAGE_BYTES) {
        throw new HttpError(400, 'Category images must be 5 MB or smaller.');
    }
    ...
    if (!detectedType || !allowedMimeTypes.has(file.mimetype) || mimeTypeFor(detectedType) !== file.mimetype) {
        throw new HttpError(400, 'Category image must be a valid JPG, PNG, or WEBP image.');
    }
    if (!env.cloudinary.cloudName || !env.cloudinary.apiKey || !env.cloudinary.apiSecret) {
        throw new HttpError(503, 'Category image storage is not configured yet.');
    }
    ...
    try {
        ...
        console.error('Cloudinary category image upload failed', error instanceof Error ? error.message : 'unknown error');
        throw new HttpError(502, 'Category image storage is temporarily unavailable.');
    }
    ...
    if (!response.ok || typeof result?.secure_url !== 'string' || typeof result.public_id !== 'string') {
        ...
        console.error('Cloudinary rejected category image upload', {
            status: response.status,
            message: providerMessage || 'unknown provider error',
        });
        throw new HttpError(502, 'The category image could not be stored. Check the image storage configuration and try again.');
    }
