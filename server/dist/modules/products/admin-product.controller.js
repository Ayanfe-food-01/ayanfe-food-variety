...
import { validateAdminProductId, validateAdminProductsQuery, validateProductFeaturedInput, validateProductFields, validateProductImageOrder, validateProductStatusInput, } from './product.validator.js';
...
const uploadedProductFiles = (request) => {
    if (!request.files || Array.isArray(request.files))
        return request.files ?? [];
    return [...(request.files.images ?? []), ...(request.files.image ?? [])];
};
const resolveImageOrder = (order, existingImages, uploadedImages) => {
    if (!order)
    ...
    for (const token of order) {
        if (token.startsWith('existing:')) {
            ...
            if (!url || !existingSet.has(url) || usedExisting.has(url))
                throw new HttpError(400, 'Product image order contains an invalid existing image.');
    ...
    if (usedUploads.size !== uploadedImages.length)
        throw new HttpError(400, 'Every uploaded product image must have an order.');
    return resolved;
};
