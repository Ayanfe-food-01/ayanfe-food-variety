...
import { cancelCustomerOrder, getCustomerOrderByNumber, listCustomerOrders } from '../services/order.service.js'
import { validateCancellationInput, validateOrderNumber } from '../validators/order.validator.js'
...
export const getCustomerOrderController: RequestHandler = async (request, response) => {
...
export const cancelCustomerOrderController: RequestHandler = async (request, response) => {
  const orderNumber = validateOrderNumber(typeof request.params.orderNumber === 'string' ? request.params.orderNumber : undefined)
  const order = await cancelCustomerOrder(
    request.authenticatedUser!.id,
    orderNumber,
    validateCancellationInput(request.body).reason,
  )
  response.json({
    success: true,
