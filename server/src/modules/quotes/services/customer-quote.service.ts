...
import { notifyQuoteAccepted } from '../email/quote.email.js'
...
export async function acceptQuoteRequest(reference: string, userId: string): Promise<QuoteRequestResponse> {
  ...
  const quoteRequest = await getCustomerQuoteRequest(reference, userId)
  await notifyQuoteAccepted(quoteRequest)
  return quoteRequest
}

/**
 * Declines a prepared or accepted quotation, moving it from QUOTED (or
 * ACCEPTED) to CANCELLED exactly once. An optional reason is stored
 * server-side for the admin's visibility and is never echoed back to the
 * customer. A customer who already accepted a quotation may still back out
 * before they place the order; once the quotation has been converted into an
 * order it can no longer be declined.
 */
export async function rejectQuoteRequest(
  ...
  if (
    existing.status !== QuoteRequestStatus.QUOTED &&
    existing.status !== QuoteRequestStatus.ACCEPTED
  ) {
    if (existing.status === QuoteRequestStatus.COMPLETED) {
      throw new HttpError(409, 'This quotation can no longer be declined because it was already converted into an order.')
    }
  ...
  await prisma.$transaction(async (transaction) => {
    const updated = await transaction.quoteRequest.updateMany({
      where: {
        id: existing.id,
        status: { in: [QuoteRequestStatus.QUOTED, QuoteRequestStatus.ACCEPTED] },
      },
      data: {
