...
export async function createQuoteRequest(
  ...
  await prisma.$transaction(async (transaction) => {
    ...
    // Delivery names are resolved from the stored location references rather
    // than trusted from the browser, and the area/city/state combination is
    // validated against the delivery-location mapping.
    let stateName = input.state ?? null
    let cityName = input.city ?? null
    if (input.fulfillmentMethod === 'DELIVERY') {
      ...
      if (!location.stateName || !location.cityName) {
        throw new HttpError(400, 'The selected delivery location could not be verified.')
      }
