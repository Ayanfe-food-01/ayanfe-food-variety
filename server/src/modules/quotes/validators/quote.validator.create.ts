...
import { isRecord, optionalEnum, optionalText, optionalUuid, requiredEmail, requiredPhone, requiredText, UUID_PATTERN } from './quote.validator.common.js'
...
export function validateCreateQuoteRequestInput(body: unknown): CreateQuoteRequestInput {
  ...
  const fulfillmentMethod = optionalEnum(body.fulfillmentMethod, 'fulfillment method', FULFILLMENT_METHODS)

  const delivery = {
    state: optionalText(body.state, 'state', 120),
    stateId: optionalUuid(body.stateId, 'state'),
    city: optionalText(body.city, 'city', 120),
    cityId: optionalUuid(body.cityId, 'city'),
    areaId: optionalUuid(body.areaId, 'area'),
    deliveryAddress: optionalText(body.deliveryAddress, 'delivery address', MAX_ADDRESS_LENGTH),
  }

  if (fulfillmentMethod === 'DELIVERY') {
    if (!delivery.stateId || (!delivery.cityId && !delivery.city)) {
      throw new HttpError(400, 'Delivery location details are required for delivery requests.')
    }
    if (!delivery.deliveryAddress) {
      throw new HttpError(400, 'A delivery address is required for delivery requests.')
    }
  } else if (fulfillmentMethod === 'PICKUP' && (delivery.stateId || delivery.cityId || delivery.areaId || delivery.deliveryAddress)) {
    throw new HttpError(400, 'Delivery details are not allowed for pickup requests.')
  }
