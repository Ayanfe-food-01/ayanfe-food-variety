...
export function validatePrepareQuotePricingInput(body: unknown): PrepareQuotePricingInput {
  ...
  const deliveryFeeModeRaw = body.deliveryFeeMode
  if (
    deliveryFeeModeRaw !== undefined
    && deliveryFeeModeRaw !== null
    && deliveryFeeModeRaw !== ''
    && deliveryFeeModeRaw !== 'ZONE'
    && deliveryFeeModeRaw !== 'FREE'
    && deliveryFeeModeRaw !== 'CUSTOM'
  ) {
    throw new HttpError(400, 'A valid delivery fee mode is required.')
  }
  const deliveryFeeMode = deliveryFeeModeRaw === undefined || deliveryFeeModeRaw === null || deliveryFeeModeRaw === ''
    ? undefined
    : (deliveryFeeModeRaw as 'ZONE' | 'FREE' | 'CUSTOM')

  if (fulfillmentMethod === FulfillmentMethod.PICKUP && deliveryFeeMode === 'ZONE') {
    throw new HttpError(400, 'The zone delivery fee mode cannot be used for pickup requests.')
  }
  if (deliveryFeeMode === 'CUSTOM' && (deliveryFee === null || deliveryFee === undefined)) {
    throw new HttpError(400, 'A delivery fee is required for the custom delivery fee mode.')
  }
  if (deliveryFeeMode === 'FREE' && deliveryFee !== null && deliveryFee !== '0.00') {
    throw new HttpError(400, 'A delivery fee cannot be locked with the free delivery fee mode.')
  }
  if (deliveryFeeMode === 'ZONE' && deliveryFee !== null) {
    throw new HttpError(400, 'A delivery fee cannot be overridden with the zone delivery fee mode.')
  }

  return { items, deliveryFee, fulfillmentMethod, deliveryFeeMode }
}